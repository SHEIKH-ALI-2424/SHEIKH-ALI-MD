...
}, 
{
  ...
  execute: async (sock, msg, args) => {
    ...
    try {
      ...
      const caption = `*🔞 SHEIKH-ALI-MD X-Video Downloader 🥵*\n\n` +
  ...
  execute: async (sock, msg, args) => {
    ...
    const contextInfo = {
      ...
      forwardedNewsletterMessageInfo: {
        ...
        newsletterName: '𒁂𓄂❥𝕚𝕥𝕩.𝑺𝑯𝑬𝑰𝑲𝑯 𝑨𝑳𝑰 🔥༽༼ ♡𒁂',
        serverMessageId: -1
      }
    };
      ...
      if (data.status && data.result && data.result.length > 0) {
        ...
        if (url.startsWith('https://www.instagram.com/reel/')) {
          if (media.url) {
            await sock.sendMessage(chatId, {
              video: { url: media.url },
              caption: "SHEIKH-ALI-MD INSTA DOWNLOADER - Video",
              contextInfo
            }, { quoted: msg });
          ...
          if (media.thumbnail) {
            await sock.sendMessage(chatId, {
              ...
              caption: "SHEIKH-ALI-MD INSTA DOWNLOADER - Image",
              contextInfo
            }, { quoted: msg });
    ...
    try {
      getFBInfo(queryURL)
        .then(async (result) => {
          ...
          await sock.sendMessage(chatId, {
            video: { url: result.hd },
            caption: "_╰►FACEBOOK VIDEO DOWNLOADED DOWNLOADED BY_ *SHEIKH-ALI-MD*",
            contextInfo
          }, { quoted: msg });
        })
        .catch(async () => {
    ...
    try {
      getFBInfo(queryURL)
        .then(async (result) => {
          ...
          await sock.sendMessage(chatId, {
            video: { url: result.sd },
            caption: "_╰►FACEBOOK VIDEO DOWNLOADED BY_ *SHEIKH-ALI-MD*",
            contextInfo
          }, { quoted: msg });
        })
        .catch(async (error) => {
      ...
      await sock.sendMessage(chatId, {
        text: "An error occurred while SHEIKH-ALI-MD was downloading your media."
      }, { quoted: msg });
    }
  }
}, 
  ...
  execute: async (sock, msg, args) => {
    ...
    try {
      ...
      if (result && !result.error) {
        ...
        await sock.sendMessage(chatId, {
          text: "A moment, SHEIKH-ALI-MD is sending your results"
        }, { quoted: msg });
...
}, 
{
  ...
  execute: async (sock, msg, args) => {
    ...
    try {
      ...
      await sock.sendMessage(chatId, {
        text: "SHEIKH-ALI-MD is sending you 5 BLACKPINK IMAGES"
      }, { quoted: msg });

      for (const imageUrl of selectedImages) {
        await sock.sendMessage(chatId, {
          image: { url: imageUrl },
          caption: "_╰►DOWNLOADED BY_ *SHEIKH-ALI-MD*",
          contextInfo
        }, { quoted: msg });
      }
...
_Enjoy using the app. © Pᴏᴡᴇʀᴇᴅ Bʏ 𒁂𓄂❥.𝑺𝑯𝑬𝑰𝑲𝑯 𝑨𝑳𝑰 🔥༽༼࿐ ♡••²⁴⁰²`,
                    contextInfo: {
                        forwardingScore: 1,
                        isForwarded: true,
                        forwardedNewsletterMessageInfo: {
